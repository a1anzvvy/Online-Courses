...
require 'pl'
require 'paths'
local dbg = require '../luadebugger/debugger'
...
opt.network = '/home/xinshuo/tmp/10703hw1/save/training/fcn_5_scratch.net' -- your trained network
opt.list_file = '../list/testLabels.txt'
opt.path_dataset = '/home/xinshuo/dataset/NYU/croptest/'
opt.codebooktxt = '../list/codebook_40.txt' 
opt.resultpath =  '/home/xinshuo/dataset/NYU/res' -- save your results here 
...
function getSamples( N, beg)
  ...
  local img_pre
  if opt.phase == "pretrain" then
    img_pre = imgs:index(2, torch.LongTensor{3, 2, 1})  
    print('testing pretrained model')
  elseif opt.phase == "scratch" then
  ...
  -- print(img_pre:size())
  inputs:resize(img_pre:size()):copy(img_pre)
  ...
  -- print(inputs:size())
  local samples = model_FCN:forward(inputs)
  -- print(samples:size())
  if opt.phase == "pretrain" then
    print('testing pretrained model')
    samples = torch.reshape(samples, N, 16, 16, 40)
  end
  if opt.phase == "scratch" then
    print('testing scratch model')
    samples = torch.reshape(samples, N, 32, 32, 40)
  end
  -- print(samples:size())
  samples = samples:transpose(2, 4)
  -- print(samples:size())
  -- local test = samples[1]:transpose(1, 3)[1][1]
  -- print(test)
  -- dbg()
